...
# ۱. تنظیمات اولیه و White-Labeling
st.set_page_config(page_title="ALMOHALENIN | Strategic Proposal", layout="wide")

st.markdown("""
    <style>
    /* حذف المان‌های اضافه برای ظاهر حرفه‌ای */
    [data-testid="stSidebar"] {display: none;}
    [data-testid="stHeader"] {display: none;}
    footer {visibility: hidden;}
    #MainMenu {visibility: hidden;}
    
    /* استایل باکس‌های متریک طلایی */
...
# --- هدر اصلی ---
st.markdown("<h1 style='text-align: center; color: #D4AF37; margin-bottom:0;'>ASSET ACQUISITION STRATEGY</h1>", unsafe_allow_html=True)
...
# --- بخش منطق سرمایه‌گذاری (Investment Logic) ---
with st.container():
    st.markdown("""
    <div class='logic-box'>
...
with input_col2:
    upfront_cash = st.slider("💰 Upfront Cash Commitment (AED)", 
                             ...
                             help="Increasing your upfront payment lowers the total asset price towards the break-even point.")
...
with c1:
    ...
    <h4 style='color: #888;'>Standard Resale</h4>
...
with c2:
    st.markdown(f"""
    <div style='background-color: #11141d; padding: 20px; border-radius: 10px; border: 1px solid #D4AF37;'>
    <h4 style='color: #D4AF37;'>ALMOHALENIN Strategy</h4>
...
labels = ["Contract Signature"] + [d.strftime("%b %y") for d in future_dates] + ["Handover"]
...
# --- تصاویر و سلب مسئولیت ---
st.divider()
col_l, col_r = st.columns([1, 1])
with col_l:
    st.markdown(f"""
    ### 🏗️ Property Highlights
    * Tower C: Prime location, high floor units.
    * Leverage: Control a 1.2M AED asset with only {(upfront_cash/current_total)*100:.1f}% down payment.
    * Efficiency: Buying at {(1 - current_total/1243000)*100:.1f}% below market average.
    """)
with col_r:
